...
public class LevelGenerator : MonoBehaviour {
	...
	public static int maxNumHoles = 3;
	...
	void Start () {
		...
		// number of holes in the maze
		int numHoles = maxNumHoles;

		// create actual maze blocks from maze boolean data
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				// flag to indicate if a hole will be spawned
				...
				if (mapData[z, x]) {
					...
					if (!characterPlaced) {
						...
						// chance to spawn a hole only if character has been placed
