...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        elseif love.keyboard.wasPressed('right') then
            ...
            gSounds['select']:play()
        elseif love.mouseClicked then
            self.boardHighlightX = math.floor((love.mouseX - self.board.x) / 32)
            self.boardHighlightY = math.floor((love.mouseY - self.board.y) / 32)
        end

        -- if we've pressed enter, to select or deselect a tile...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') or love.mouseClicked then
            ...
            elseif not self.board:canMove(self.highlightedTile, x, y) then
                ...
                :finish(function()
                    ...
                    while not self.board:existsMatch() do
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for kk, tile in pairs(match) do
                -- each tile add 50 point by variety
