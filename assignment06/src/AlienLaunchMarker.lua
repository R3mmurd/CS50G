...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.keyboard.wasPressed('space') and not self.alienSplit and not self.alienCollided then
            self.alienSplit = true

            local mainAlien = self.aliens[1]
            local xPos, yPos = mainAlien.body:getPosition()
            local velX, velY = mainAlien.body:getLinearVelocity()

            -- spawn upper alien
            local upperAlien = Alien(self.world, 'round', xPos, yPos, 'Player')
            table.insert(self.aliens, upperAlien)

            upperAlien.body:setLinearVelocity(velX, velY - 40)
            ...
            -- spawn lower alien
            local lowerAlien = Alien(self.world, 'round', xPos, yPos, 'Player')
            table.insert(self.aliens, lowerAlien)
 
            lowerAlien.body:setLinearVelocity(velX, velY + 25)
